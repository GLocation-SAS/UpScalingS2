  ...
  (async () => {
    try {
      ...
      const result = await executeWithAuthRetry(async () => {
        ...
        const visualized = imageResult.visualize({
          bands: ['B4', 'B3', 'B2'],
          min: 0,
          max: 3000
        });

        const downloadUrl = await getDownloadUrl(visualized, {
          name: 'sentinel_image',
          region: roi,
          scale: 10,
          format: 'GEO_TIFF'
        ...
        console.log('📤 URL de descarga obtenida, enviando a cloud function de procesamiento...');
        ...
        if (!processingResponse.ok) {
          ...
          throw new Error(`Error en cloud function de procesamiento: ${processingResponse.status} - ${errorText}`);
        }
        ...
        return {
          url: tileUrl,
          jpegUrl: processingResult.jpegUrl,
          jpegFileName: processingResult.jpegFileName,
          geotiffUrl: processingResult.geotiffUrl,
          geotiffFileName: processingResult.geotiffFileName,
